...
struct argo_transport {
	...
	/*
	 * TEMPORARY instrumentation, dumped once in release(). Brackets the whole
	 * of stream_enqueue(), so comparing tx_ns against the ring's hypercall
	 * time says whether the per-message cost is inside Xen or around it.
	 */
	u64 tx_msgs;
	u64 tx_ns;
	u64 tx_ns_max;
                                      ...
                                      struct vsock_sock* psk)
{
	...
	vsk->trans = kzalloc(sizeof(struct argo_transport), GFP_KERNEL);
	if (!vsk->trans) {
		return -ENOMEM;
	}
...
static void argo_transport_release(struct vsock_sock* vsk)
{
	...
	if (t->tx_msgs)
		pr_info("socket dom%u:%u stats: %llu sendmsg, avg %llu ns "
		        "(max %llu ns), %llu attempts/msg, %zu bytes\n",
		        vsk->local_addr.svm_cid, vsk->local_addr.svm_port,
		        t->tx_msgs, t->tx_ns / t->tx_msgs, t->tx_ns_max,
		        t->tx_attempts / t->tx_msgs, t->sent_bytes);
        ...
        int flags)
{
	 ...
	 * Only relevant once the worker has actually parked an skb - kicking it
	 * after every recv() costs a worker wakeup per message for nothing.
	 */
        ...
        struct msghdr* msg, size_t len)
{
	...
	for (;;) {
		...
		lock_sock(sk);
		/*
		 * Deduct what was actually slept, not a flat millisecond: the sleep
		 * above is ~100us, so the old accounting retired a SO_SNDTIMEO ten
		 * times too fast. MAX_SCHEDULE_TIMEOUT - a blocking socket - has to
		 * stay pinned, or the loop invents a deadline the caller never set.
		 */
	...
	dt = ktime_get_ns() - t0;
	argo_trans(vsk)->tx_msgs++;
	argo_trans(vsk)->tx_ns += dt;
	argo_trans(vsk)->tx_attempts += attempts;
	if (dt > argo_trans(vsk)->tx_ns_max)
		argo_trans(vsk)->tx_ns_max = dt;
...
static irqreturn_t argo_interrupt(int irq, void* dev_id)
{
	...
	u64 now = argo_lat_trace ? ktime_get_ns() : 0;
	...
	list_for_each_entry_safe(h, tmp, &argo_rings, l) {
		if (argo_ring_has_data(h) >=
		    sizeof(struct xen_argo_ring_message_header)) {
			/*
			 * Keep the first kick of a burst: if the worker has not run
			 * since, that is the instant the delay should be measured from.
			 */
